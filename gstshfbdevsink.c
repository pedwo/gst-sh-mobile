...
static GstFlowReturn gst_shfbdevsink_render (GstBaseSink * bsink,
    GstBuffer * buff);
...
static void gst_shfbdevsink_get_property (GObject * object,
...
static void
gst_shfbdevsink_base_init (gpointer g_class)
{
  ...
  static GstStaticPadTemplate sink_template = GST_STATIC_PAD_TEMPLATE ("sink",
      GST_PAD_SINK,
      GST_PAD_ALWAYS,
      GST_STATIC_CAPS (GST_SHFBDEV_TEMPLATE_CAPS)
      );
...
static GstCaps *
gst_shfbdevsink_getcaps (GstBaseSink * bsink)
{
  GstSHFBDEVSink *fbdevsink;
  GstCaps *caps;
  uint32_t rmask;
...
static gboolean
gst_shfbdevsink_setcaps (GstBaseSink * bsink, GstCaps * vscapslist)
{
  GstSHFBDEVSink *fbdevsink;
...
gst_shfbdevsink_render (GstBaseSink * bsink, GstBuffer * buf)
{
...
static gboolean
gst_shfbdevsink_start (GstBaseSink * bsink)
{
  GstSHFBDEVSink *fbdevsink;
...
static gboolean
gst_shfbdevsink_stop (GstBaseSink * bsink)
{
  GstSHFBDEVSink *fbdevsink;
...
static void
gst_shfbdevsink_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstSHFBDEVSink *fbdevsink;
...
{
  if (!gst_element_register (plugin, "shfbdevsink", GST_RANK_NONE,
          GST_TYPE_SHFBDEVSINK))
    return FALSE;
...
static void
gst_shfbdevsink_class_init (GstSHFBDEVSinkClass * klass)
{
  ...
  gstelement_class->change_state =
      GST_DEBUG_FUNCPTR (gst_shfbdevsink_change_state);
...
static void
gst_shfbdevsink_finalize (GObject * object)
{
...
gst_shfbdevsink_get_type (void)
{
  ...
  if (!fbdevsink_type) {
    ...
    fbdevsink_type =
        g_type_register_static (GST_TYPE_BASE_SINK, "GstSHFBDEVSink",
        &fbdevsink_info, 0);
  }
