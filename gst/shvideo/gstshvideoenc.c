...
static void gst_shvideo_enc_init(GstshvideoEnc * shvideoenc, GstshvideoEncClass * gklass)
{
	...
	gst_pad_set_activatepull_function(shvideoenc->sinkpad, gst_shvideo_enc_activate_pull);
...
gboolean gst_shvideoenc_set_src_caps(GstshvideoEnc * shvideoenc)
{
	...
	if (shvideoenc->format == SHCodecs_Format_MPEG4) {
		caps = gst_caps_new_simple("video/mpeg", "width", G_TYPE_INT,
					   ...
					   shvideoenc->fps_denominator, "mpegversion", G_TYPE_INT,
					   4, NULL);
...
void gst_shvideo_enc_init_encoder(GstshvideoEnc * shvideoenc)
{
	...
	shvideoenc->encoder = shcodecs_encoder_init(shvideoenc->width,
							shvideoenc->height, shvideoenc->format);

	shcodecs_encoder_set_input_callback(shvideoenc->encoder, gst_shvideo_enc_get_input,
						shvideoenc);
	shcodecs_encoder_set_output_callback(shvideoenc->encoder, gst_shvideo_enc_write_output,
						 shvideoenc);

	ret = GetFromCtrlFtoEncParam(shvideoenc->encoder, &shvideoenc->ainfo);
	...
	shcodecs_encoder_set_frame_no_increment(shvideoenc->encoder,
						...
						(shvideoenc->encoder) /
						(shcodecs_encoder_get_frame_rate
						 (shvideoenc->encoder) / 10));
	...
	GST_DEBUG_OBJECT(shvideoenc, "Encoder init: %ldx%ld %ldfps format:%ld",
			 ...
			 shcodecs_encoder_get_frame_rate(shvideoenc->encoder) /
			 10, shcodecs_encoder_get_stream_type(shvideoenc->encoder));
}
...
static gboolean gst_shvideo_enc_activate_pull(GstPad * pad, gboolean active)
{
	...
	if (active) {
		enc->offset = 0;
		return gst_pad_start_task(pad, (GstTaskFunction) gst_shvideo_enc_loop, enc);
	} else {
...
static void gst_shvideo_enc_loop(GstshvideoEnc * enc)
{
	...
	ret = gst_pad_pull_range(enc->sinkpad, enc->offset, yuv_size, &enc->buffer_yuv);
...
gboolean gst_shvideo_enc_plugin_init(GstPlugin * plugin)
{
	if (!gst_element_register
		(plugin, "gst-sh-mobile-enc", GST_RANK_PRIMARY, GST_TYPE_SHVIDEOENC))
		return FALSE;
