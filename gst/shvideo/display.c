	...
	int v4l_fmt)
{
	...
	aspect_x = (double) pvt->fb_var.xres / (double) w;
	aspect_y = (double) pvt->fb_var.yres / (double) h;
	...
	/* Assuming 2 bytes per pixel */
	fb_addr = (unsigned long)pvt->fb_screenMem
				+ (pvt->fb_var.xres - dst_w)
				+ (pvt->fb_var.yres - dst_h) * pvt->fb_var.xres;
